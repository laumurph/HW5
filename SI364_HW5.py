...
## DONE: Create database and change the SQLAlchemy Database URI.
## Your Postgres database should be your uniqname, plus HW5, e.g. "jczettaHW5" or "maupandeHW5"
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://localhost/laumurphHW5"
...
# DONE: Run commands to create your migrations folder and get ready to create a first migration, as shown in the textbook and in class.
...
def send_async_email(app, msg):
    with app.app_context():
        mail.send(msg) #use the mail application to send the email
...
# DONE: Add and run a  migration so that each twitter username also saves an associated email.
# User model
class User(db.Model):
    __tablename__ = "users"
...
# DONE: Add a field in the form for a user to enter their email (the email that goes with the twitter username entered).
# DONE: Edit the template that renders the form so that email is also asked for!
class TweetForm(FlaskForm):
    ...
    email = StringField("What is your email for this account?", validators=[Required()])
...
def get_or_create_user(db_session, username, user_email):
    ...
    if user:
        ...
        user = User(twitter_username=username, email=user_email)
        db_session.add(user)
        db_session.commit()
        return user
...
def get_or_create_tweet(db_session, input_text, username, user_email):
    ...
    if tweet:
        ...
        user = get_or_create_user(db_session, username, user_email)
